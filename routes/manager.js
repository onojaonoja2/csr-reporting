...
router.post('/csr/remove/:csrId', (req, res) => {
  db.prepare("UPDATE users SET isActive = 0, removedBy = ?, removedAt = datetime('now') WHERE id = ?")
...
router.get('/aggregate', (req, res) => {
  const dates = db.prepare("SELECT DISTINCT date FROM sales_entries ORDER BY date DESC").all();
  const allData = dates.map(d => {
    const entries = db.prepare("SELECT se.*, u.fullName AS csrName, u.phoneNumber, u.state FROM sales_entries se LEFT JOIN users u ON se.csrId = u.id WHERE se.date = ? ORDER BY u.fullName").all(d.date);
    const dayData = entries.map(e => {
      ...
      return { ...e, items, totalValue: items.reduce((s, i) => s + i.salesValue, 0) };
    });
    return { date: d.date, entries: dayData };
  });
